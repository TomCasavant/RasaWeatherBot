	...
	def blueprint(self, on_new_message):
		...
		def event():
			...
			if request.json.get('token') == self.slack_client and request.json.get('type') == 'event_callback':
				data = request.json
				messaging_event = data.get('event')
				channel = messaging_event.get('channel')
				user = messaging_event.get('user')
				text = messaging_event.get('text')
				bot = messaging_event.get('bot_id')
				if bot == None:
